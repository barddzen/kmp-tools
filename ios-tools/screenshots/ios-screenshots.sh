...
# Run from any iOS Xcode project directory
#
# Usage:
#   ios-screenshots.sh --analyze      # Analyze and generate config
#   ios-screenshots.sh --generate     # Generate from config
#   ios-screenshots.sh --auto         # Both in one shot
#   ios-screenshots.sh --auto --preserve-names  # Skip auto-ordering
...
if ! git rev-parse --is-inside-work-tree > /dev/null 2>&1; then
    ...
    echo "Run this from an iOS project directory"
    exit 1
fi
...
if [ ! -d "$SCREENSHOTS_DIR" ]; then
    ...
    echo "Create with: mkdir -p assets/screenshots/Input/{ios_iphone,ios_ipad}"
    exit 1
fi
