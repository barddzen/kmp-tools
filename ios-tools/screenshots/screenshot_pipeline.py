#!/usr/bin/env python3
"""
Automated Screenshot Pipeline for iOS App Store Submissions
...
Usage:
    ...
    python screenshot_pipeline.py --regenerate ios_iphone/01_main.png ios_ipad/02_detail.png
"""
...
class ScreenshotPipeline:
    """Main screenshot automation pipeline for iOS"""
    ...
    def analyze(self, preserve_names=False, force_files=None):
        ...
        """
        logger.info("=" * 70)
        logger.info("iOS SCREENSHOT ANALYZER - Powered by Claude Vision")
        logger.info("=" * 70)
        ...
        for platform_dir, (platform, device_type) in platforms.items():
            ...
            if not platform_path.exists():
                logger.warning(f"Skipping {platform_dir} - directory not found")
                continue
            ...
            if not image_files:
                logger.warning(f"No images found in {platform_path}")
                continue
            ...
            if preserve_names:
                ...
                for result in ranked_results:
                    ...
                    if matched:
                        ...
                        logger.warning(f"  File not found: {original_name}")
        ...
        logger.info("  3. Run: ios-screenshots.sh --generate")

        if not preserve_names:
            logger.info("")
            logger.info("To preserve original filenames next time:")
            logger.info("   ios-screenshots.sh --analyze --preserve-names")
    ...
    def generate(self, regenerate_files=None):
        ...
        """
        logger.info("=" * 70)
        logger.info("iOS SCREENSHOT GENERATOR")
        logger.info("=" * 70)
        logger.info("")
        ...
        if not self.config_file.exists():
            ...
            logger.error("Run analysis first: ios-screenshots.sh --analyze")
            sys.exit(1)
        ...
        for platform, platform_config in config['platforms'].items():
            ...
            for screenshot in screenshots:
                ...
                if 'rank' in screenshot:
                    hero_badge = " HERO" if screenshot.get('hero_shot') else ""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Automated Screenshot Pipeline for iOS App Store',
...
Auto-Ordering:
  ...
  Use --preserve-names to skip auto-ordering.
    ...
    parser.add_argument(
        ...
        nargs='+',
        metavar='FILE',
        help='Regenerate specific screenshots (e.g., ios_iphone/01_main.png)'
    )
